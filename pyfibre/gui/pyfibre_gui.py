...
from tkinter import (
    BooleanVar, DoubleVar, IntVar, Frame,
    TclError, Button, OUTSIDE, DISABLED,
    NORMAL, Tk, HORIZONTAL
)
...
from .process_run import run_analysis
...
class PyFibreGUI:
    ...
    def __init__(self, master, n_proc):
        ...
        "Initiate program log, queue and input file list"
        ...
        self.run_button.grid(column=0, row=2, columnspan=3)

        self.stop_button = Button(
            self.file_display, width=2 * button_w, text="STOP",
            command=self.stop_run, state=DISABLED)
        self.stop_button.grid(column=2, row=2, columnspan=2)

        self.progress = Progressbar(
            self.file_display, orient=HORIZONTAL, length=400,
            mode='determinate')
        self.progress.grid(column=0, row=3, columnspan=5)
    ...
    def write_run(self):
        ...
        self.file_display.progress['maximum'] = n_files
