...
def distance_sum(vector, boundary):
    ...
    # from boundary
    cost = np.nansum([np.abs(vec - boundary) for vec in vector])
...
def binary_classifier_spherical(centres, intensities, boundary):
    ...
    mask = [all(vec <= boundary) for vec in vector]
...
class SHGPLTransKmeansFilter(BaseKmeansFilter):
    def cellular_classifier(
        self, label_image, centres, boundary=(0.65, 1.1, 1.40, 0.92)
    ):
        ...
        # boundary
        mask, cost = binary_classifier_spherical(centres, intensities, boundary)
