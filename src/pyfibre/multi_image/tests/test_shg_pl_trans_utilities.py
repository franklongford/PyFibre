...
from pyfibre.multi_image.shg_pl_trans_utilities import (
    extract_prefix,
    get_files_prefixes,
    filter_input_files,
    get_image_type,
...
class TestSHGPLTransUtils(TestCase):
    def setUp(self):
        self.input_files = [
    ...
    def test_get_image_type(self):
        self.assertEqual("SHG-PL-Trans", get_image_type(self.input_files[0]))
        self.assertEqual("PL-Trans", get_image_type(self.input_files[2]))
        self.assertEqual("SHG", get_image_type(self.input_files[3]))
    ...
    def test_extract_prefix(self):
        self.assertEqual(
            f"{self.prefix}1", extract_prefix(self.input_files[0], "-pl-shg")
        )
        self.assertEqual(self.prefix, extract_prefix(self.input_files[2], "-pl"))
        self.assertEqual(self.prefix, extract_prefix(self.input_files[3], "-shg"))

    def test_get_files_prefixes(self):
        input_files = [self.input_files[0], self.input_files[2], self.input_files[3]]

        files, prefixes = get_files_prefixes(input_files, "SHG")
        self.assertListEqual([input_files[2]], files)
        self.assertListEqual([self.prefix], prefixes)

        files, prefixes = get_files_prefixes(input_files, "PL-Trans")
        self.assertListEqual([input_files[1]], files)
        self.assertListEqual([self.prefix], prefixes)

        files, prefixes = get_files_prefixes(input_files, "SHG-PL-Trans")
        self.assertListEqual([input_files[0]], files)
        self.assertListEqual([f"{self.prefix}1"], prefixes)
